...
# ==================================================================== CONFIG
MODE       = "mlp"                     # architecture this file measures
RESUME     = True
...
def quad_scalar(m,p,b,x,delta,micro):
    ...
    return total/B
def grad_quad(m,p,b,x,delta,micro):    # m_l = delta^T (d_l F) delta
    ...
    for (regime,act) in cells:
        for w in GEO_WIDTHS:
            try:
                ...
                del sds; torch.cuda.empty_cache() if DEVICE=="cuda" else None
            except Exception as e:
