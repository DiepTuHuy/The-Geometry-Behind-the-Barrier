#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Damping sweep for the geodesic deviation, sp regime (MLP/MNIST).

     python lambda_sweep_sp.py
...
Output
------
  lam_sweep_mlp_sp.csv        one row per pair, resumable
  lam_sweep_mlp_sp_cell.csv   per-cell medians
  plus a closing report in the log: the exponent at each of the three lambdas,
  the per-pair drop, mu_eff/lambda, and a verdict.

Required on disk
----------------
  ckpt_pmlp_v2/sp_{{act}}_w{{w}}_s{{s}}.pt and MNIST under ./data
...
MODE      = "mlp"
RUN_TAG   = "pmlp_v2"
REGIMES   = ["sp"]                # the only line that differs between the three files
